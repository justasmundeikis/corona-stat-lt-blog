...
df <- read.csv("https://raw.githubusercontent.com/tomasdzedulionis/Data/master/Eksportas.csv", header=TRUE, encoding="UTF-8", stringsAsFactors = FALSE)
Sys.setlocale("LC_ALL","Lithuanian")
df <- df%>% 
...
Sys.setlocale("LC_ALL","Lithuanian")
ggplot(df, aes(x=reorder(Šalys.ir.teritorijos.Pavadinimas, -Proc), y=Proc))+
...
Sys.setlocale("LC_ALL", "english")
usman <- Quandl("ISM/MAN_PMI")
usman <- add_row(usman, Date="2020-04-01", PMI=48.5)
usman <- usman %>%filter(Date>="2008-01-01")
usman$Date <- as.Date(usman$Date, format="%Y-%m-%d")
usman$Date <- usman$Date %m+% months(-1)
...
plot( usgdp$DATE, usgdp$GDPC1_PC1, type="l", lwd=3, col="black",
      main="BVP JAV 2008-2020m.", sub="Duomenų šaltiniai: IHS Markit, ISM, Federal Reserve Bank, skaičiavimai: Corona-stat.lt", xlab="Metai", ylab="BVP augimas, %", xlim=c(2008,2019.5))
abline(h=0, untf=FALSE, lty=3)
legend(2011,23 ,c("Gamybinis PMI","Paslaugų PMI", "Bendras PMI"),lty=c(1,1),bty = "n",lwd=3,col=c("black","steelblue", "grey"), cex = 1, horiz = TRUE)
dev.off()
