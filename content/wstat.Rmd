...
Žemiau pateikiama atrinkta statistika, kuri leidžia geriau įvertinti COVID-19 viruso paplitimą pasaulyje.
...
Žemiau pateikiamas naujų registruotų atvejų skaičiaus santykinis pokytis (procentais) per paskutinę parą (`r max.date-1` - `r max.date` ). Žemėlapyje atvaizduojamos tik šalys turinčios daugiau nei 500 regsitruotų atvejų ir daugiau nei 5% dienos augimą Šis žemėlapis iliustruoja aktyviųs koronaviruso židinius pasaulyje.
...
ggplot(df, aes(x=date, y=value, fill=var))+
  ...
  labs(title=paste0("Pasaulinis koronaviruso atvejų skaičius ir pasiskirstymas, ", max.date),
       subtitle = "Šaltinis: JHCSSE, skaičiavimai: Corona-Stat.lt",
       x="Data",
       y="Skaičius")+
...
ggplot(df, aes(x=date, y=value, color=valstybe, group=valstybe)) +
  ...
  labs(
    title="Užsikrėtimų skaičius daugiausia patvirtintų atvejų turinčiose šalyse (6-15 vietos)",
    subtitle="Šaltinis: JHCSSE, skaičiavimai: Corona-Stat.lt",
    x="Laikotarpis",
    y="Užsikrėtimų skaičius"
  )+
...
ggplot(df, aes(x=date, y=diff)) +
  geom_area()+
  scale_x_date(breaks="1 month")+
  labs(title=paste0('Dieninis mirčių skaičius skaičius (top 6 pagal mirčių sk.), ',
                    max.date),
       subtitle="Šaltinis: JHCSSE, skaičiavimai: Corona-Stat.lt",
       x="Data",
       y="Skaičius") +
