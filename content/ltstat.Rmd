...
* iš NVSC imtinai iki `r max(data_lt$date_confirmation)`
...
ggplot(df, aes(fill=cases))+
  ...
  scale_fill_gradientn(colours=brewer.pal(name ="YlOrRd", n=9),
                       ...
                       trans="log",
                       na.value = "grey100")+
  labs(title="Per vakar patvrintų atvėjų skačius",
       subtitle = "Šaltinis: NVSC, skaičiavimai: Corona-Stat.lt")
...
ggplot(df, aes(age_group,value, fill=sex))+
  geom_bar(stat="identity",
           position="dodge")+
  scale_fill_brewer(palette="Set1")+
  labs(title="Užsikrėtimai pagal amžiaus kategorijas, paskutinės 14d.",
       subtitle="Šaltinis: NVSC, skaičiavimai: Corona-Stat.lt",
       x="Amžiaus grupė", 
       y="Užsikrėtimų skaičius")+
  theme(legend.title=element_blank(),
